...
const BsFormInput = class {
  constructor(hostRef) {
    ...
    this.getValidator = () => {
      if (this.type === InputType.Email) {
        return validators_types.ValidatorNames.Email;
      }
      if (!isEmpty.isEmpty(this.min) || !isEmpty.isEmpty(this.max)) {
        if (this.charInputTypes.includes(this.type)) {
          return { name: validators_types.ValidatorNames.Length, options: { min: this.min, max: this.max } };
        }
        return { name: validators_types.ValidatorNames.MinMax, options: { min: this.min, max: this.max } };
      }
      return this.type;
    };
    this.charInputTypes = [
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("bs-form-field", { ref: (el) => {
        this.formFieldRef = el;
      }, value: this.value, name: this.name, hidden: this.type === 'hidden', focused: this.placeholderInputTypes.includes(this.type) ? true : this.focused, filled: this.filled, invalid: this.invalid, required: this.required, invalidMessage: this.invalidMessage, requiredMessage: this.requiredMessage, label: this.label, icon: this.icon, validator: this.getValidator(), onClick: this.focus }, this.type === 'textarea' ? (index.h("textarea", { rows: this.rows, slot: "form-element", class: { 'form-input': true, 'is-resizeable': this.resizeable }, required: this.required, id: this.name, minlength: this.min, maxlength: this.max, value: this.value, name: this.name, onFocus: this.focus, onFocusout: this.focusout })) : (index.h("input", Object.assign({ ref: (el) => {
        this.inputEl = el;
      }, slot: "form-element", class: "form-input", required: this.required, id: this.name, type: this.type }, minMaxArg, { value: this.value, name: this.name, onFocus: this.focus, onFocusout: this.focusout }))), index.h("label", { slot: "form-label", class: "form-label", htmlFor: this.name, title: this.label }, this.label, this.required ? index.h("span", { "aria-hidden": "true" }, "\u00A0*") : ''))));
  }
...
exports.bs_form_input = BsFormInput;

//# sourceMappingURL=bs-form-input.cjs.entry.js.map