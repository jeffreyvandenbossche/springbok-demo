...
const BsFormSelect = class {
  ...
  render() {
    var _a, _b;
    return (index.h(index.Host, { focused: this.focused }, index.h("bs-form-field", { ref: (el) => {
        this.formFieldRef = el;
      }, value: (_a = this.value) === null || _a === void 0 ? void 0 : _a.value, name: this.name, focused: this.focused || this.filled, filled: this.filled, invalid: this.invalid, required: this.required, requiredMessage: this.requiredMessage, label: this.label, icon: this.icon }, index.h("button", { slot: "form-element", ref: (el) => {
        this.selectWrap = el;
      }, id: this.name, class: "custom-select", onFocus: this.focus }, index.h("span", { class: "custom-select-value" }, (_b = this.value) === null || _b === void 0 ? void 0 : _b.text), index.h("select", { ref: (el) => {
        this.selectElRef = el;
        return this.selectElRef;
      }, class: "hidden", required: this.required, id: `hidden-${this.name}`, name: this.name }, this.parsedOptions.map((option) => (index.h("option", { selected: option.selected, value: option.value, disabled: option.disabled }, option.text)))), index.h("div", { "aria-labelledby": this.name }, index.h("ul", { class: "custom-options-wrapper hidden", "aria-haspopup": "listbox", "aria-expanded": "false", role: "group" }, this.parsedOptions.map((option) => (index.h("li", { ref: (el) => {
        this.customOptions.push(el);
      }, role: "option", class: `custom-option ${option.disabled ? 'is-disabled' : ''}`, tabindex: option.disabled ? '-1' : '0', "aria-disabled": option.disabled }, option.text)))))), index.h("label", { slot: "form-label", class: "form-label", htmlFor: this.name, title: this.label }, this.label, this.required ? index.h("span", { "aria-hidden": "true" }, "\u00A0*") : ''))));
  }
...
exports.bs_form_select = BsFormSelect;

//# sourceMappingURL=bs-form-select.cjs.entry.js.map