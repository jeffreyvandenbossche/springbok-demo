...
const BsFormCheckbox = class {
  ...
  get parsedOptions() {
    return parseIfString.parseIfString(this.options);
  }
  componentDidLoad() {
    const selected = [];
  ...
  render() {
    return (index.h(index.Host, null, index.h("fieldset", { class: "form-fieldset" }, index.h("legend", { class: "form-legend" }, this.label, this.required && !isEmpty.isEmpty(this.label) ? index.h("span", { "aria-hidden": "true" }, "\u00A0*") : ''), index.h("bs-form-field", { ref: (el) => {
        this.formFieldRef = el;
      }, value: this.value.toString(), name: this.name, invalid: this.invalid, required: this.required, requiredMessage: this.requiredMessage, label: this.label, border: false }, this.parsedOptions.map((checkbox, index$1) => (index.h("div", { slot: "form-element", class: "form-group" }, index.h("input", { class: "form-input", id: `checkbox-${index$1}`, type: "checkbox", value: checkbox.value || 'true', disabled: checkbox.disabled, checked: checkbox.selected, onChange: this.handleOnChange }), index.h("label", { class: "form-label", htmlFor: `checkbox-${index$1}`, title: checkbox.text }, checkbox.text))))))));
  }
};
__decorate([
  memoize.Memoize('options')
], BsFormCheckbox.prototype, "parsedOptions", null);
BsFormCheckbox.style = formCheckboxCss;

exports.bs_form_checkbox = BsFormCheckbox;

//# sourceMappingURL=bs-form-checkbox.cjs.entry.js.map