...
const BsBreadcrumb = class {
  ...
  get parsedData() {
    return parseIfString.parseIfString(this.data);
  }
  componentWillLoad() {
    this.displayedItems = [...this.parsedData];
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("nav", { "aria-label": "breadcrumb" }, index.h("ol", { class: "breadcrumb" }, this.displayedItems.map((breadcrumbItem, index$1) => {
      const isFirstItem = index$1 === 0;
      const isLastItem = index$1 === this.displayedItems.length - 1;
      return typeof breadcrumbItem !== 'string' ? (index.h("li", null, index.h("bs-click-action", { tagClass: `breadcrumb-item ${isLastItem ? 'is-active' : ''} ${isFirstItem ? 'is-b-mark' : ''}`, href: breadcrumbItem.url, "aria-current": isLastItem ? 'page' : '' }, index.h("bs-icon", { name: isFirstItem ? icon_types.IconName.BMark : icon_types.IconName.CaretForward }), !isFirstItem && index.h("span", null, breadcrumbItem.pageTitle)))) : (index.h("li", null, index.h("bs-click-action", { tagClass: 'breadcrumb-item', name: this.action, onBsClickAction: this.onBsButtonClick }, index.h("bs-icon", { name: icon_types.IconName.CaretForward }), index.h("span", null, "..."))));
    })), index.h("script", { type: "application/ld+json" }, JSON.stringify(schema)))));
  }
};
__decorate([
  memoize.Memoize('data')
], BsBreadcrumb.prototype, "parsedData", null);
BsBreadcrumb.style = breadcrumbCss;

exports.bs_breadcrumb = BsBreadcrumb;

//# sourceMappingURL=bs-breadcrumb.cjs.entry.js.map