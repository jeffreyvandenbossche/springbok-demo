...
const BsExperienceZoneModal = class {
  constructor(hostRef) {
    ...
    this.bsPauseYoutubeAction = index.createEvent(this, "bsPauseYoutubeAction", 7);
  ...
  get parsedExperienceZoneData() {
    return parseIfString.parseIfString(this.experienceZoneData);
  }
  /** Listeners */
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { class: "experience-zone-modal", ref: (el) => {
        this.experienceZoneModalRef = el;
      } }, this.parsedExperienceZoneData.stories.map((story, index$1) => (index.h("bs-experience-zone-story", { active: index$1 === this.activeIndex, hideNavigation: this.parsedExperienceZoneData.stories.length < 2, storyData: story }))))));
  }
};
__decorate$3([
  memoize.Memoize('experienceZoneData')
...
const BsImageCarousel = class {
  constructor(hostRef) {
    ...
    /** methods */
    this.onResize = () => {
      this.desktop = breakpoints.compareBreakpoint('min', breakpoints.Breakpoint.md);
    };
  ...
  get parsedImageCollection() {
    return parseIfString.parseIfString(this.imageCollection);
  }
  handleIndexChange() {
  ...
  handleBsSwipeAction(e) {
    if (e.detail.direction === defaultLayout_types.TouchDirections.Left) {
      this.overlayRef.nextSlide();
    }
    if (e.detail.direction === defaultLayout_types.TouchDirections.Right) {
...
__decorate$2([
  memoize.Memoize('imageCollection')
...
const BsOverlayMenu = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.modalId = baseModal_types.BaseModalId.OverlayMenu;
    /**
     * Methods
     */
  ...
  render() {
    return (index.h(index.Host, null, index.h("bs-base-modal", { class: "base-modal", ref: (el) => {
        this.modalRef = el;
      }, modalId: this.modalId, transitionType: "fade" }, index.h("div", { class: "overlay-menu" }, index.h("div", { class: `overlay-menu__container ${this.isClosing ? 'is-closing' : ''}` }, index.h("div", { class: "overlay-menu__head" }, breakpoints.compareBreakpoint('max', breakpoints.Breakpoint.md) ? (index.h("bs-image", Object.assign({ class: "overlay-menu__logo" }, this.parsedHeaderNavigation.mobileMainLogo, { caption: "" }))) : (index.h("bs-image", Object.assign({ class: "overlay-menu__logo" }, this.parsedHeaderNavigation.mainLogo, { caption: "" }))), index.h("button", { class: "overlay-menu__close-button", "aria-label": "Close", onClick: this.handleCloseButtonClick.bind(this) }, index.h("bs-icon", { name: icon_types.IconName.Close }))), index.h("div", { class: "overlay-menu__content" }, index.h("bs-accordion", null, this.parsedHeaderNavigation.navigation.secondary &&
      this.parsedHeaderNavigation.navigation.secondary.map((item, index$1) => (index.h("bs-click-action", { slot: "accordion-item", tagClass: "overlay-menu__link", href: isEmpty.isEmpty(item.children) ? item.href : '', target: item.target, disabled: !isEmpty.isEmpty(item.children) }, index.h("bs-accordion-item", { identifier: `primary-navigation-item-${index$1}`, "accordion-item-title": item.title, expandable: Boolean(item.children && item.children.length > 0) }, index.h("bs-accordion", { slot: "accordion-content" }, item.children &&
        item.children.map((child, childIndex) => (index.h("bs-click-action", { slot: "accordion-item", class: "overlay-menu__link", tagClass: "overlay-menu__link", target: item.target, href: isEmpty.isEmpty(child.children) ? child.href : '', disabled: !isEmpty.isEmpty(child.children) }, index.h("div", { class: "overlay-menu__thumbnail-item" }, index.h("bs-image", Object.assign({}, child.thumbnail, { caption: "" })), index.h("bs-accordion-item", { identifier: `primary-navigation-child-item-${childIndex}`, "accordion-item-title": child.title, expandable: Boolean(child.children && child.children.length > 0) })))))))))), this.parsedHeaderNavigation.navigation.primary &&
      this.parsedHeaderNavigation.navigation.primary.map((item, index$1) => (index.h("bs-click-action", { slot: "accordion-item", tagClass: "overlay-menu__link", href: isEmpty.isEmpty(item.children) ? item.href : '', target: item.target, disabled: !isEmpty.isEmpty(item.children) }, index.h("bs-accordion-item", { identifier: `secondary-navigation-item-${index$1}`, "accordion-item-title": item.title, expandable: Boolean(item.children && item.children.length > 0) }, index.h("bs-accordion", { class: "overlay-menu__accordion-child", slot: "accordion-content" }, item.children &&
        item.children.map((child, childIndex) => (index.h("bs-click-action", { slot: "accordion-item", tagClass: "overlay-menu__link", href: isEmpty.isEmpty(child.children) ? child.href : '', target: item.target, disabled: !isEmpty.isEmpty(item.children) }, index.h("bs-accordion-item", { identifier: `primary-navigation-child-item-${childIndex}`, "accordion-item-title": child.title, expandable: Boolean(child.children && child.children.length > 0) })))))))))), this.parsedHeaderNavigation.stickyButton &&
      this.parsedHeaderNavigation.stickyButton.text && (index.h("bs-button", Object.assign({}, this.parsedHeaderNavigation.stickyButton, { isFullWidth: true, iconRight: icon_types.IconName.ArrowForward }))), this.parsedHeaderNavigation.navigation.sliderItems && (index.h("bs-slider", { class: "overlay-menu__slider" }, this.parsedHeaderNavigation.navigation.sliderItems.map((item) => (index.h("bs-click-action", { class: "overlay-menu__slider-item", slot: "slider-item", href: item.href, target: item.target }, index.h("bs-product-card", { "card-image": JSON.stringify(item.thumbnail), "card-title": item.title, "card-text": item.description }))))))))))));
  }
};
__decorate$1([
...
const BsTestBadgeModal = class {
  constructor(hostRef) {
    ...
    this.updateIndex = (newIndex) => {
      if (!isEmpty.isEmpty(this.parsedtestBadgeCollection[newIndex])) {
        this.index = newIndex;
      }
      else {
  ...
  get parsedtestBadgeCollection() {
    return parseIfString.parseIfString(this.testBadgeCollection);
  }
  /** Listeners */
  bsNextSlideActionHandler() {
    this.updateIndex(this.index + 1);
  ...
  render() {
    return (index.h(index.Host, null, index.h("bs-light-overlay", { ref: (el) => {
        this.overlayRef = el;
      }, screens: this.parsedtestBadgeCollection.length > 1 }, index.h("bs-test-badge-detail", Object.assign({}, this.parsedtestBadgeCollection[this.index], { slot: "media" })))));
  }
};
__decorate([
  memoize.Memoize('testBadgeCollection')
], BsTestBadgeModal.prototype, "parsedtestBadgeCollection", null);
BsTestBadgeModal.style = testBadgeModalCss;

exports.bs_experience_zone_modal = BsExperienceZoneModal;
exports.bs_image_carousel = BsImageCarousel;
exports.bs_overlay_menu = BsOverlayMenu;
exports.bs_test_badge_modal = BsTestBadgeModal;

//# sourceMappingURL=bs-experience-zone-modal_4.cjs.entry.js.map